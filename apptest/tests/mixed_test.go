...
func TestMixedPrometheusQueries(t *testing.T) {
	...
	gotAdminTenantsResponse := vmselect.APIV1AdminTenants(t)
...
func TestMixedDeleteSeries(t *testing.T) {
	...
	vmselect.APIV1AdminTSDBDeleteSeries(tc.T(), `{__name__=~"metric1.*"}`, apptest.QueryOpts{
		Tenant: tenantID1,
	})
	apptest.AssertSeries(tc, vmsingle, "metric.*", "", start, end, data2.WantSeries)
	vmselect.APIV1AdminTSDBDeleteSeries(tc.T(), `{__name__=~"metric2.*"}`, apptest.QueryOpts{
		Tenant: tenantID2,
	})
	apptest.AssertSeries(tc, vmsingle, "metric.*", "", start, end, data2.WantSeries)
	vmselect.APIV1AdminTSDBDeleteSeries(tc.T(), `{__name__=~"metric2.*"}`, apptest.QueryOpts{
